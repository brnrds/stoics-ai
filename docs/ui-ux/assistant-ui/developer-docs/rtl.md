...
`# shadcn primitives under components/ui/ npx shadcn@latest migrate rtl # assistant-ui wrappers (pass a custom glob) npx shadcn@latest migrate rtl 'components/assistant-ui/**/*.tsx'`

Commit the diff and do not re-run. The upstream migration is not fully idempotent on repeat runs (

- href

  https\://github.com/shadcn-ui/ui/pull/9891

\#9891

); you may end up with duplicated `rtl:translate-x-*` classes.

After migrating, follow the setup below.

## [Setup](#setup)

### [1. Install the `direction` component](#1-install-the-direction-component)
...
### [2. Set `dir` on your root element](#2-set-dir-on-your-root-element)
...
### [3. Wrap your app with `DirectionProvider`](#3-wrap-your-app-with-directionprovider)

- title

  app/providers.tsx

`"use client"; import { DirectionProvider } from "@/components/ui/direction"; export function Providers({ children }: { children: React.ReactNode }) { return <DirectionProvider dir="rtl">{children}</DirectionProvider>; }`
